...
# sys.path.insert(0, os.path.join(curr_path, '../3rdparty/mxnet', config.MXNET_VERSION))
...
def train_net(args, ctx, pretrained, pretrained_flow, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    ...
    sym = sym_instance.get_train_symbol_impression(config)
...
#     # rec
#     record_file = None
#     video_index_dict = None
#     if config.dataset.from_rec:
#         record_file = mx.recordio.MXIndexedRecordIO(config.dataset.rec_idx, config.dataset.rec_data, 'r')
...
#         train_data = AnchorLoaderOffline(feat_sym, roidb, config, rec_file=record_file, video_index_dict=video_index_dict, from_rec=config.dataset.from_rec, batch_size=input_batch_size,
#                                          shuffle=config.TRAIN.SHUFFLE, ctx=ctx, feat_stride=config.network.RPN_FEAT_STRIDE, anchor_scales=config.network.ANCHOR_SCALES,
...
#     # infer max shape
#     max_data_shape = [('data_oldkey', (config.TRAIN.BATCH_IMAGES, 3, max([v[0] for v in config.SCALES]), max([v[1] for v in config.SCALES]))),
#                       ('data_cur', (config.TRAIN.BATCH_IMAGES, 3, max([v[0] for v in config.SCALES]), max([v[1] for v in config.SCALES]))),
#                       ('data_newkey', (config.TRAIN.BATCH_IMAGES, 3, max([v[0] for v in config.SCALES]), max([v[1] for v in config.SCALES]))),
#                       ('eq_flag_key2key', (1,)),
#                       ('eq_flag_key2cur', (1,))]
...
#     lr_scheduler = WarmupMultiFactorScheduler(lr_iters, lr_factor, config.TRAIN.warmup, config.TRAIN.warmup_lr, config.TRAIN.warmup_end_lr, config.TRAIN.warmup_step, config.TRAIN.warmup_linear)
#     # optimizer
